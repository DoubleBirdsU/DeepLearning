...
class Model(base_model):
    ...
    @staticmethod
    def _make_conv(mdef, in_ch=None):
        modules = nn.Sequential()

        bn = mdef['batch_normalize']  # 1 or 0 / use or not
        filters = mdef['filters']  # out_channels
        kernel_size = mdef['size']  # kernel size
        stride = mdef['stride'] if 'stride' in mdef else (mdef['stride_y'], mdef['stride_x'],)
        padding = kernel_size // 2 if mdef['pad'] else 0
        groups = mdef['groups'] if 'groups' in mdef else 1
        activation = mdef['activation'] if 'activation' in mdef else 'relu'
